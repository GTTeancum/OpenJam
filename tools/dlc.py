...
def cmd_import(args):
    ...
    # EA's last roster update sits beside the data folder rather than in it,
    # so the loop above never saw it - and every published mod bundles one,
    # because every published mod's rosters name the players in it. It is
    # converted and kept beside the data, where the game looks for it by
    # name. See tools/roster_update.py.
    update = src / roster_update.NAME
    players = 0
    if update.is_file():
        try:
            players = roster_update.install(str(update), str(dest), quiet=True)
        except Exception as exc:                    # noqa: BLE001
            print("  ! the bundled roster update would not convert ({})".format(exc))
    ...
    if players:
        print("  the bundled roster update came too: {} texture(s) converted"
              .format(players))
...
def cmd_instance(args):
    ...
    for p, rel in rel_files(GAME_ROOT):
        ...
        if rel in overrides:
            ...
            base += 1
    # The roster update sits beside the game's data rather than inside it,
    # which is where the game looks for it. See tools/roster_update.py.
    if mod_path:
        update = mod_path / roster_update.NAME
        if update.is_file():
            link_or_copy(update, out / roster_update.NAME)
